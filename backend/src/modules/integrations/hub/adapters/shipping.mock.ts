...
export class ShippingMockAdapter extends MockAdapterBase {
  ...
  capabilities(): AdapterCapability[] {
    return [
      { operation: 'label.create', method: 'write', description: 'Crear guía / etiqueta de envío' },
      { operation: 'tracking.get', method: 'read', description: 'Consultar estado por tracking' },
  ...
  protected operationHandlers() {
    return {
      'label.create': this.wrap((args) => this.labelCreate(args), 'label.create'),
      'tracking.get': this.wrap((args) => this.trackingGet(args), 'tracking.get'),
  ...
  private labelCreate(args: Record<string, unknown>) {
    ...
    const to = args.to as { name?: string; address?: string; city?: string; country?: string };
  ...
  private trackingGet(args: Record<string, unknown>) {
    ...
    const stages = ['PICKED_UP', 'IN_TRANSIT', 'CUSTOMS_CLEARANCE', 'OUT_FOR_DELIVERY', 'DELIVERED'];
    return {
      tracking,
      status: stages[phase],
